...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createStartup() {
    ...
    const body = { ...req.body };
    if (body?.capital_requirement?.length > 0) {
      const create_capital_requirement = body.capital_requirement;
      body.capital_requirement = {
        create: create_capital_requirement,
      };
    } else {
      delete body.capital_requirement;
    }
