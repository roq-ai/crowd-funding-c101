...
import { getInvestments, deleteInvestmentById } from 'apiSdk/investments';
import { InvestmentInterface } from 'interfaces/investment';

type ColumnType = ColumnDef<InvestmentInterface, unknown>;
...
export function InvestmentListPage(props: InvestmentListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InvestmentInterface>>(
    () => `/investments?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: InvestmentInterface) => {
    if (hasAccess('investment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/investments/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/investments/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('investment', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/investments/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('investment', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="investment" />
        </Flex>

        {hasAccess('investment', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/investments/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
