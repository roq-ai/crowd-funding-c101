...
import { getInvestments } from 'apiSdk/investments';
import { InvestmentInterface } from 'interfaces/investment';
import { getCapitalRequirements } from 'apiSdk/capital-requirements';
import { CapitalRequirementInterface } from 'interfaces/capital-requirement';
import { InvestmentListPage as ShowCaseList } from 'pages/investments';

function DashboardPage() {
  const { session } = useSession();

  const investmentStat = useSWR<PaginatedInterface<InvestmentInterface>>(
    () => `/investments`,
    () => getInvestments({ limit: 1, offset: 0 }),
  );

  const capitalRequirementStat = useSWR<PaginatedInterface<CapitalRequirementInterface>>(
    () => `/capital-requirements`,
    () => getCapitalRequirements({ limit: 1, offset: 0 }),
  );

  const statResults = [
    {
      title: 'Investments',
      result: investmentStat?.data?.totalCount,
      isLoading: investmentStat.isLoading,
    },
    {
      title: 'Capital Requirements',
      result: capitalRequirementStat?.data?.totalCount,
      isLoading: capitalRequirementStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`investments`} passHref legacyBehavior>
                  <Button
